...
#%%
df = pd.read_excel('/home/docker/ecom_analysis/ecom.xlsx', sheet_name='E Comm')
df_c = df[df['Churn'] == 1]
df_nc = df[df['Churn'] == 0]
...
for idx,col in enumerate(df_cv.columns[2:]):
    sns.kdeplot(data=df_cv[col][df_cv['Churn'] == 1], label='Churned', linewidth=3, ax=ax[idx] )
    sns.kdeplot(data=df_cv[col][df_cv['Churn'] == 0], label='No Churn', linewidth=3, ax=ax[idx])
plt.show()
